...
## Application to walkDevin

```{r}
#Step 1 + 2
df <- Walk3
folder_name <- "walkDevin"
...
start_time <-   as.POSIXct("2025-02-17 10:42:45.833")
...
data_walkDevin <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkDevin <- df_all_from_mvm_alt_nofilter(data_walkDevin, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkDevin, 0.8, folder_name)
...
shiny_walkDevin <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkDevin[[1]], server = shiny_walkDevin[[2]])
```

## Example of finding a precise starting time, applied to walkDevin

```{r}
#step 1
raw_df <- Walk3
raw_folder_name <- "walkDevin"
...
#Step 2
#estimated_start_time_raw <- as.POSIXct("2025-02-17 10:42:56.000")
estimated_start_time_raw <-   as.POSIXct("2025-02-17 10:42:55.833")
...
shiny_walkDevin_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkDevin_raw[[1]], server = shiny_walkDevin_raw[[2]])
```
...
## Example of finding an estimated starting time, applied to walkDevin

```{r}
temp <- filter(Walk3, time > as.POSIXct("2025-02-17 10:43:05.000"), time <  as.POSIXct("2025-02-17 10:43:07.000"))
plot(temp$Y, type = 'l')
```
