...
## Application to walkElvira

```{r}
#Step 1 + 2
df <- Walk2
folder_name <- "walkElvira"
...
start_time <-   as.POSIXct("2025-01-20 15:00:31.599999")
...
data_walkElvira <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkElvira <- df_all_from_mvm_alt_nofilter(data_walkElvira, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkElvira, 0.8, folder_name)
...
shiny_walkElvira <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkElvira[[1]], server = shiny_walkElvira[[2]])
```

## Example of finding a precise starting time, applied to walkElvira

```{r}
#step 1
raw_df <- Walk2
raw_folder_name <- "walkElvira"
...
#Step 2
estimated_start_time_raw <- as.POSIXct("2025-01-20 15:00:31.000")
...
shiny_walkElvira_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkElvira_raw[[1]], server = shiny_walkElvira_raw[[2]])
```

```{r}
frames_difference <- -18
...
## Example of finding an estimated starting time, applied to walkElvira

```{r}
temp <- filter(Walk2, time > as.POSIXct("2025-01-20 15:03:02.000"), time <  as.POSIXct("2025-01-20 15:03:15.000"))
plot(temp$Y, type = 'l')
```
