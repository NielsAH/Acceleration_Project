...
## Application to walkPleun

```{r}
#Step 1 + 2
df <- Walk1
folder_name <- "walkPleun"
...
start_time <- as.POSIXct("2025-01-20 10:50:06.533333")
...
data_walkPleun <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkPleun <- df_all_from_mvm_alt_nofilter(data_walkPleun, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkPleun, 0.8, folder_name)
...
shiny_walkPleun <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkPleun[[1]], server = shiny_walkPleun[[2]])
```

## Example of finding a precise starting time, applied to walkPleun

```{r}
#step 1
raw_df <- Walk1
raw_folder_name <- "walkPleun"
...
#Step 2
estimated_start_time_raw <- as.POSIXct("2025-01-20 10:50:07.000")
...
shiny_walkPleun_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkPleun_raw[[1]], server = shiny_walkPleun_raw[[2]])
```
...
## Example of finding an estimated starting time, applied to walkPleun

```{r}
raw_df <- Walk1
temp <- filter(Walk1, time > as.POSIXct("2025-01-20 10:56:30.500"), time <  as.POSIXct("2025-01-20 10:56:40.500"))
plot(temp$Z, type = 'l')
```
