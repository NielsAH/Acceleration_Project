...
## Application to walkGerwin

```{r}
#Step 1 + 2
df <- Walk3
folder_name <- "walkGerwin"
...
start_time <-   as.POSIXct("2025-02-17 11:15:49.500")
...
data_walkGerwin <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkGerwin <- df_all_from_mvm_alt_nofilter(data_walkGerwin, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkGerwin, 0.8, folder_name)
...
shiny_walkGerwin <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkGerwin[[1]], server = shiny_walkGerwin[[2]])
```

## Example of finding a precise starting time, applied to walkGerwin

```{r}
#step 1
raw_df <- Walk3
raw_folder_name <- "walkGerwin"
...
#Step 2
estimated_start_time_raw <- as.POSIXct("2025-02-17 11:15:59.500")
...
shiny_walkGerwin_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkGerwin_raw[[1]], server = shiny_walkGerwin_raw[[2]])
```
...
## Example of finding an estimated starting time, applied to walkGerwin

```{r}
temp <- filter(Walk3, time > as.POSIXct("2025-02-17 11:16:05.000"), time <  as.POSIXct("2025-02-17 11:16:12.000"))
plot(temp$Y, type = 'l')
```
