...
## Application to walkEstelle

```{r}
#Step 1 + 2
df <- Walk1
folder_name <- "walkEstelle"
...
start_time <-  as.POSIXct("2025-01-20 11:37:03.666")
...
data_walkEstelle <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkEstelle <- df_all_from_mvm_alt_nofilter(data_walkEstelle, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkEstelle, 0.8, folder_name)
...
shiny_walkEstelle <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkEstelle[[1]], server = shiny_walkEstelle[[2]])
```

## Example of finding a precise starting time, applied to walkEstelle

```{r}
#step 1
raw_df <- Walk1
raw_folder_name <- "walkEstelle"
dir.create(file.path("frames", raw_folder_name, "video_raw"), recursive = TRUE, showWarnings = FALSE)

start_vid_raw <- "00:04:05"
length_vid_raw <- "15"
cmd <- paste0(
...
#Step 2
estimated_start_time_raw <- as.POSIXct("2025-01-20 11:41:09.000")
...
shiny_walkEstelle_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkEstelle_raw[[1]], server = shiny_walkEstelle_raw[[2]])
```

```{r}
frames_difference <- 10
...
## Example of finding an estimated starting time, applied to walkEstelle

```{r}
raw_df <- Walk1
start_time <-  as.POSIXct("2025-01-20 11:36:30.000")
#Actual start time should be around 11:37:04 ?!?! Is it even correct for walk2 ??? And for the rest???
#But this new start time makes no sense for starting walking, so maybe it is just wrong? Or does it make sense?
temp <- filter(Walk1, time > as.POSIXct("2025-01-20 11:44:19.000"), time <  as.POSIXct("2025-01-20 11:44:29.000"))
plot(temp$Z, type = 'l')
```
