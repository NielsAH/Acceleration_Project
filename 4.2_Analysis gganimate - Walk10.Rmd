...
## Application to walkMike

```{r}
#Step 1 + 2
df <- Walk3
folder_name <- "walkMike"
...
data_walkMike <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkMike <- df_all_from_mvm_alt_nofilter(data_walkMike, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkMike, 0.8, folder_name)
...
shiny_walkMike <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkMike[[1]], server = shiny_walkMike[[2]])
```

## Example of finding a precise starting time, applied to walkMike

```{r}
#step 1
raw_df <- Walk3
raw_folder_name <- "walkMike"
...
shiny_walkMike_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkMike_raw[[1]], server = shiny_walkMike_raw[[2]])
```
...
## Example of finding an estimated starting time, applied to walkMike
