...
## Application to walkSpasm

```{r}
#Step 1 + 2
df <- Misc3
folder_name <- "walkSpasm"
...
cmd <- paste0("ffmpeg -ss 00:00:00 -t 3334 -i frames/", folder_name, "/", folder_name, ".mp4 -q:v 3 -vf \"fps=30,scale=280:420\" frames/", folder_name, "/video/frame_%05d.jpg")
system(cmd)
...
start_time <-   as.POSIXct("2025-04-15 11:34:57.099999")
...
#As allowing as possible. See if still gets steps!
wdw_size <- 120
ma_data <- 5 
ma_energy <- 5 
...
data_walkSpasm <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkSpasm <- df_all_from_mvm_alt_nofilter(data_walkSpasm, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkSpasm, 0.8, folder_name)
...
shiny_walkSpasm <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkSpasm[[1]], server = shiny_walkSpasm[[2]])
```

## Example of finding a precise starting time, applied to walkSpasm

```{r}
#step 1
raw_df <- Misc3
raw_folder_name <- "walkSpasm"
dir.create(file.path("frames", raw_folder_name, "video_raw"), recursive = TRUE, showWarnings = FALSE)

start_vid_raw <- "00:00:00"
length_vid_raw <- "50"
cmd <- paste0(
  "ffmpeg -i frames/", raw_folder_name, "/", raw_folder_name, ".mp4 ",
  " -ss ", start_vid_raw, 
  " -t ", length_vid_raw, 
  " -q:v 1 -vf \"fps=30,scale=400:600\" frames/", 
  raw_folder_name, "/video_raw/frame_%05d.jpg"
)
system(cmd)

#Step 2
estimated_start_time_raw <- as.POSIXct("2025-04-15 11:34:57.500")
...
shiny_walkSpasm_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkSpasm_raw[[1]], server = shiny_walkSpasm_raw[[2]])
```

```{r}
frames_difference <- 12
...
## Example of finding an estimated starting time, applied to walkSpasm

```{r}
temp <- filter(Misc3, time > as.POSIXct("2025-04-15 11:33:30.000"), time <  as.POSIXct("2025-04-15 11:36:30.000"))
plot(temp$Y, type = 'l')
temp <- filter(Misc3, time > as.POSIXct("2025-04-15 11:34:57.500"), time <  as.POSIXct("2025-04-15 11:35:03.000"))
plot(temp$Y, type = 'l')
```
