...
## Application to walkRoel

```{r}
#Step 1 + 2
df <- Walk3
folder_name <- "walkRoel"
...
start_time <-   as.POSIXct("2025-02-17 11:46:37.500")
...
data_walkRoel <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkRoel <- df_all_from_mvm_alt_nofilter(data_walkRoel, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkRoel, 0.8, folder_name)
...
shiny_walkRoel <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkRoel[[1]], server = shiny_walkRoel[[2]])
```

## Example of finding a precise starting time, applied to walkRoel

```{r}
#step 1
raw_df <- Walk3
raw_folder_name <- "walkRoel"
dir.create(file.path("frames", raw_folder_name, "video_raw"), recursive = TRUE, showWarnings = FALSE)

start_vid_raw <- "00:01:10"
length_vid_raw <- "10"
cmd <- paste0(
...
#Step 2
estimated_start_time_raw <- as.POSIXct("2025-02-17 11:47:48.000")
...
shiny_walkRoel_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkRoel_raw[[1]], server = shiny_walkRoel_raw[[2]])
```

```{r}
frames_difference <- 15
...
## Example of finding an estimated starting time, applied to walkRoel

```{r}
temp <- filter(Walk3, time > as.POSIXct("2025-02-17 11:47:47.000"), time <  as.POSIXct("2025-02-17 11:47:51.000"))
plot(temp$Y, type = 'l')
```
