...
4. Find the precise starting time of the video, e.g. the observation that matches the first frame in the video. Easiest to do this is by working with part of the video and raw data to see when it matches best. A semi-automated way to do this is given below. _Add here how to do this precisely_
...
1. ...
2. ... _estimated starting time, should preferably be within half a second already for the approach to be feasible_
3. ...

## Finding an estimate for estimated starting time
1. ...
2. ...
3. ...
...
```{r}
shinyApp(ui = shiny_walkRoccoNew[[1]], server = shiny_walkRoccoNew[[2]])
...
## Example of the approach, applied to walkRoccoNew

```{r}
#Step 1 + 2
df <- Walk1
folder_name <- "walkRoccoNew"
...
data_walkRoccoNew <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkRoccoNew <- df_all_from_mvm_alt_nofilter(data_walkRoccoNew, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkRoccoNew, 0.8, folder_name)
...
shiny_walkRoccoNew <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
## Example of finding a precise starting time, applied to walkRoccoNew

```{r}
#step 1
raw_df <- Walk1
raw_folder_name <- "walkRoccoNew"
...
shiny_walkRoccoNew_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkRoccoNew_raw[[1]], server = shiny_walkRoccoNew_raw[[2]])
```
...
## Example of finding an estimated starting time, applied to walkRoccoNew
