...
#Below are not to tailor to fit, but because in originally extracting the stairs I took smaller windows to definitely be correct. Here, I update them to really contain 14 steps instead of e.g. 12
temp$stairs[(stairs_up_idx_1[1] - 30):stairs_up_idx_1[1]] <- 2
temp$stairs[stairs_up_idx_1[2]:(stairs_up_idx_1[2] + 10)] <- 2
temp$stairs[(stairs_up_idx_2[1] - 40):stairs_up_idx_2[1]] <- 2
temp$stairs[(stairs_up_idx_2[2]):(stairs_up_idx_2[2]+28)] <- 2
```

```{r}
#Taking out a small part of the movement
start_idx <- 271539
stop_idx <- 272150
periodicity_values <- abs_periodicity_calculator(df_part_trim, 3, start_idx, stop_idx, 18, 5)

temp[start_idx:stop_idx,] %>% filter(idx >= start_idx & idx <= stop_idx) %>% ggplot(aes(x = time)) + geom_line(aes(y = -Y), colour = "blue")

temp[start_idx:stop_idx,] %>% filter(idx >= start_idx & idx <= stop_idx) %>% ggplot(aes(x = time)) +
  ...
  geom_line(aes(y = 0.04*-Y), colour = "blue")
```
