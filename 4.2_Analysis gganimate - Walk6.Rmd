...
## Application to walkSverre

```{r}
#Step 1 + 2
df <- Walk2
folder_name <- "walkSverre"
...
data_walkSverre <- generate_data(folder_name,
                width, 
                start_time, 
                df,
                wdw_size,
...
steps_info_walkSverre <- df_all_from_mvm_alt_nofilter(data_walkSverre, 200, min_square_difference_vert)

generate_images_avg(steps_info_walkSverre, 0.8, folder_name)
...
shiny_walkSverre <- generate_server(vid_frames,
                         folder_name,
                         streamvid,
                         stream1,
                         stream2,
...
shinyApp(ui = shiny_walkSverre[[1]], server = shiny_walkSverre[[2]])
```

## Example of finding a precise starting time, applied to walkSverre

```{r}
#step 1
raw_df <- Walk2
raw_folder_name <- "walkSverre"
dir.create(file.path("frames", raw_folder_name, "video_raw"), recursive = TRUE, showWarnings = FALSE)

start_vid_raw <- "00:00:30"
length_vid_raw <- "15"
cmd <- paste0(
...
#Step 2
estimated_start_time_raw <- as.POSIXct("2025-01-20 15:37:59.000")
estimated_start_time_raw <- estimated_start_time_raw_new
...
shiny_walkSverre_raw <- generate_server(raw_vid_frames,
                         raw_folder_name,
                         raw_streamvid,
                         raw_stream1,
                         raw_stream2,
...
shinyApp(ui = shiny_walkSverre_raw[[1]], server = shiny_walkSverre_raw[[2]])
```

```{r}
frames_difference <- 22
...
## Example of finding an estimated starting time, applied to walkSverre

```{r}
temp <- filter(Walk2, time > as.POSIXct("2025-01-20 15:40:19.000"), time <  as.POSIXct("2025-01-20 15:40:40.000"))
plot(temp$Y, type = 'l')
```
